...
- **Білий екран на КОНКРЕТНІЙ Wi-Fi, але працює на мобільному інтернеті** → це мережа, а не бот.
  `server.tail7f319a.ts.net` має дві адреси: публічну funnel-ingress Tailscale (`185.40.234.x`,
  бачить відкритий інтернет) і внутрішню tailnet (`100.68.224.73`, лише для пристроїв у Tailscale).
  Якщо Wi-Fi віддає телефону внутрішню `100.x` (свій DNS/MagicDNS) або блокує funnel-сервери —
  запит не долітає (у лозі `aiohttp.access` НЕ буде iOS-UA, лише локальні тести). Фікс: на телефоні
  DNS цієї Wi-Fi → `1.1.1.1`/`8.8.8.8`; якщо не помогло — мережа блокує файрволом, користуватись через
  мобільний інтернет. Діагностика: `curl -s 'https://dns.google/resolve?name=<host>&type=A'` має дати
  публічні `185.40.234.x`; локальний `Resolve-DnsName` на сервері дає внутрішню `100.x` (Tailscale-перехоплення).
